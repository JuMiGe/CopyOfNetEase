...
import com.jumige.mobile.news.adapter.NewsImgPagerViewAdapter;
...
public class HeadNewsCoverMark extends RelativeLayout implements Runnable {
	...
	private List<View> threadViews = new ArrayList<View>();
	// 指示当前正在显示第几张图
	...
	private NewsImgPagerViewAdapter adapter;

	// 作为一个能够在xml布局文件中直接使用的View，必须重写拥有Context和
	// AttributeSet参数的构造函数：

	public HeadNewsCoverMark(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.HeadNewsCoverMark, 0, 0);

		try {
			dotsViewHeight = a.getDimension(
					R.styleable.HeadNewsCoverMark_dotsViewHeight, 40);
			// 这里依次获取所有的属性值，此处省略，可参看最后附上的全部代码

			dotsSpacing = a.getDimension(

			R.styleable.HeadNewsCoverMark_dotsSpacing, 2);
			dotsBgAlpha = a.getDimension(

			R.styleable.HeadNewsCoverMark_dotsBgAlpha, 0.5f);
			changeInterval = (int) a.getDimension(

			R.styleable.HeadNewsCoverMark_changeInterval, 1000);
			dotsFocusImage = a.getDrawable(

			R.styleable.HeadNewsCoverMark_dotsFocusImage);
			dotsBlurImage = a.getDrawable(

			R.styleable.HeadNewsCoverMark_dotsBlurImage);
			dotsBackground = a.getDrawable(

			R.styleable.HeadNewsCoverMark_dotsBackground);
		...
		thread.start();
		adapter = new NewsImgPagerViewAdapter(mContext, views, scaleType);
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			adapter.notifyDataSetChanged0(threadViews);
